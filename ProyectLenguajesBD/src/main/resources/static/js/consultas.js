document.addEventListener('DOMContentLoaded', () => {
    ...
    const fechaNueva = document.getElementById('fechaConsulta');
    const fechaEditar = document.getElementById('editarFecha');
    if (fechaNueva) fechaNueva.setAttribute('min', hoy);
    if (fechaEditar) fechaEditar.setAttribute('min', hoy);
    ...
    async function cargarConsultas() {
        ...
        tablaConsultas.innerHTML = consultas.map(consulta => `
            <tr data-id="${consulta.id}" 
                data-paciente-id="${consulta.pacienteId}" 
                data-cita-id="${consulta.citaId}" 
                data-medico-id="${consulta.medicoId}" 
    ...
    buscarPaciente.addEventListener('keyup', function () {
        ...
        tablaConsultas.querySelectorAll('tr').forEach(fila => {
    ...
    // Eventos de la tabla (ver, editar, eliminar)
    tablaConsultas.addEventListener('click', async event => {
        ...
        if (boton.dataset.action === 'ver') {
            ...
            document.getElementById('editarFecha').value = fila.cells[0].textContent; // fecha actual
            ...
            document.getElementById('editarDiagnostico').value = fila.dataset.diagnostico;
    ...
    document.getElementById('actualizarConsulta').addEventListener('click', async () => {
        ...
        const datos = {
            ...
            diagnostico: document.getElementById('editarDiagnostico').value,
            observaciones: document.getElementById('editarObservaciones').value
        };
        await fetch(`/api/consultas/${consultaEnEdicion.dataset.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(datos)
        });
    ...
    // Filtro de citas según paciente
